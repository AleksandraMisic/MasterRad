...
namespace FTN.Services.NetworkModelService
{
	public class NetworkModel
	{
        ...
        private void RepackDelta(Delta delta)
        {
            ...
            pomList = delta.InsertOperations
                            .Where(rd => this.EntityExistsMrid(rd.Id, rd.GetProperty(ModelCode.IDOBJ_MRID).ToString()) == true)
                            .ToList();
        ...
        private void ResetReferences(Delta delta)
        {
            foreach (ResourceDescription rd in delta.InsertOperations)
            {
                foreach (Property p in rd.Properties)
                {
                    if (p.Type == PropertyType.Reference)
                    {
                        ...
                        ResourceDescription reference = new ResourceDescription();
                        //ResourceDescription reference = delta.InsertOperations.Where(r => r.Id == res).FirstOrDefault();
                        //if (reference == null)
                        //{
                            reference = delta.UpdateOperations.Where(r => r.Id == res).FirstOrDefault();
                        //}
